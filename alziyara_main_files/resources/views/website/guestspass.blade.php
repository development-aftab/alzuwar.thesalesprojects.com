        ...
        <div class="container">
            <div class="row">
                <div class="col-lg-12 Alziyara text-center" data-aos="fade-right" data-aos-duration="3000">
                    <h3>{!!($pages->where('slug','guestpasses')->first()->title??'Not Available') !!}</h3>
                    {!!($pages->where('slug','guestpasses')->first()->description??'Not Available') !!}
                </div>
        ...
        <div class="container">
            <!--<div class="row">
                <div class="col-md-12 col-bg-color">
                    {{--<form role="form" id="form_transport_details" class="margin-bottom-0" method="post" action="{{ route('searchguestpass') }}">--}}
                            ...
                            <div class="form-group col-md-5">
                                <div class="input-group" id="loc-from">
                                    ...
                                    <select class="form-control" name="city" required>
                                        ...
                                        @foreach($cityNames as $city_name)
                                            <option value="{{$city_name->GuestPassLocation}}">{{$city_name->GuestPassLocation}} </option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>-->
                ...
                <div class="col-md-8 pb-4">
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item"><a href="{{route('guestspasses')}}"> Shrine Programs</a></li>
                            @if($route_name == 'searchguestpass')
                                <li class="breadcrumb-item active" aria-current="page">{{$search_city}}</li>
                            @endif
                            {{--<li class="breadcrumb-item"><a href="#"> Iraq Package</li>--}}
                            {{--<li class="breadcrumb-item"><a href="#"> Iraq</a></li>--}}
                            {{--<li class="breadcrumb-item active" aria-current="page">Ashura In Karabala 1443</a></li>--}}
                        </ol>
                    </nav>
                </div>
                ...
                <div class="col-md-12">
                    <ul class="nav nav-tabs" role="tablist">
                        <li class="nav-item view_all">
                            <a class="nav-link active" href="{{ URL::to("/guestspasses/")}}" role="tab">View All</a>
                        </li>
                        @if($route_name != 'searchguestpass')
                            @foreach($cityNames as $city_name)
                                {{--<li class="nav-item"  onclick="getGuestPassBYCity('{{$cityName->GuestPassLocation}}')">--}}
                                <li class="nav-item">
                                    <a class="nav-link @if($route_name == 'guestspassesbycity/'.$city_name->GuestPassLocation) active @endif" href="{{ URL::to("/guestspassesbycity/")}}/{{$city_name->GuestPassLocation}}" role="tab">{{ucfirst($city_name->GuestPassLocation)}}</a>
                                </li>
                            @endforeach
                        @endif
                        {{--<li class="nav-item">--}}
                        {{--<a class="nav-link" data-toggle="tab" href="#tabs-2" role="tab">Najaf</a>--}}
                        {{--</li>--}}
                        {{--<li class="nav-item">--}}
                        {{--<a class="nav-link" data-toggle="tab" href="#tabs-3" role="tab">Kadhmain</a>--}}
                        {{--</li>--}}
                        {{--<li class="nav-item">--}}
                        {{--<a class="nav-link" data-toggle="tab" href="#tabs-4" role="tab">Baghdad</a>--}}
                        {{--</li>--}}
                        {{--<li class="nav-item">--}}
                        {{--<a class="nav-link" data-toggle="tab" href="#tabs-5" role="tab">Samarrah</a>--}}
                        {{--</li>--}}
                    </ul>

                    <!-- Tab panes -->
                    <div class="tab-content" id="guestpass_card_section">
                        @include('website.guestpass_card_section')
                    </div>
                </div>
            </div>
        </div>
...
@push('js')
    ...
    <script>
        ...
        {{--$.ajax({--}}
        {{--type: "GET",--}}
        {{--url: '{{ URL::to('guestspassesbycity/'+cityName)}}',--}}
...
//            alert($("#sort_by").val());
            @if($route_name == 'guestspasses')
                $.get('{{ URL::to("guestpass-sorting")}}/' + $("#sort_by").val(), function (data) {
            ...
            @else
            $.get('{{ URL::to("guestpass-sorting-with-city")}}/'+$("#sort_by").val()+'/{{$cityName}}',function(data){
                $('#guestpass_card_section').empty();
                $('#guestpass_card_section').html(data);
            });
            $('#guestpass_card_section').on('click', '.pagination a', function (e) {
