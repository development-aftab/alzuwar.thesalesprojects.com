...
@section('content')
    ...
    @if(session('message'))
        ...
        <div class="account-title">
            ...
            <p class="alert alert-success text-center">{{session('message')}}
                @if(session('message') == "Need the link of resending email confirmation as it could get lost in emails.")
                    <span class="d-block  mt-4">Resend Email Confirmation<a href="{{url('registration-Resend-Mail')}}" class="login"> Resend Email</a></span>
                @endif
            </p>
        </div>
    @elseif(session('error'))
        <div class="account-title">
            {{--<p class="alert alert-success">{{session('message')}}</p>--}}
            <p class="alert alert-danger text-center">{{session('error')}}
                @if(session('error') == "Your AlZuwar Account is not Verified Kindly check your email!!")
                    <span class="d-block  mt-4">Resend Email Confirmation<a href="{{url('registration-Resend-Mail')}}" class="login"> Resend Email</a></span>
                @endif
            </p>
        </div>
            ...
            <div class="inner_col">
                <form method="post" action="{{ route('login') }}">
                    ...
                    <div class="mb-4 icon_position">
                        <div class="icon"><img src="{{ asset('website') }}/img/Icon awesome-envelope.png" alt=""></div>
                        <input type="email" placeholder="Email Address" aria-label="Username" aria-describedby="basic-addon1" name="email" value="{{ old('email') }}" required autofocus>
                    </div>
                    <div class="mb-4 icon_position">
                    ...
                    <div class="d-flex justify-content-between">
                        <div class="mb-4 form-check">
                            <input type="checkbox" class="" id="checkbox" name="remember" {{ old('remember') ? 'checked' : '' }}>
                    ...
                    <div class="buttons text-center mt-4">
                        ...
                        <a href="{{url('user-signup')}}" class="d-block signup">Sign Up</a>
                    </div>
                </form>
            </div>
        </div>
