    ...
    <div class="row">
        <div class="col-sm-12">
            <div class="white-box">
                ...
                <a class="btn btn-success pull-right" href="{{URL('myrooms')}}/{{$data['room']['PropertyId']??0}}">View All Rooms</a>
                {{--<br/>--}}
                <div class="clearfix"></div>
                <hr>
                <!--<p class="text-muted m-b-30 font-13"> All bootstrap element classies </p>-->
                <form  class="form-horizontal" method="POST" action="{{route('myupdaterooms')}}" enctype='multipart/form-data'  >
                ...
                {{csrf_field()}}
                    ...
                    <div class="row">
					<input type="hidden" name="roomid" class="form-control" value="{{ $data['room']->id }}" required >
                        <label class="col-md-2">Room Name/No</label>
                        <div class="col-md-4">
                            <input type="text" name="RoomName" class="form-control" value="{{old('RoomName', $data['room']->RoomName)}}" required placeholder="Room Name/No">
						</div>
							...
							<div class="col-md-4">
								<select class="form-control" name="roomproperty" required >
									@foreach($data['property'] as $myproperty)
										<option value="{{$myproperty->PropertyID}}" @if($myproperty->PropertyID == $data['room']->PropertyId) selected @endif >{{$myproperty->Name}}</option>
									@endforeach
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-md-4">
                            {{--<input type="number" name="RoomPrice" class="form-control" value="{{  old('RoomPrice', $data['room']->Price) }}" placeholder="Room Price" required>--}}
                            <div class="input-group">
                                <span class="input-group-addon"><i class="fa fa-dollar"></i></span>
                                <input type="number" name="RoomPrice" class="form-control" value="{{number_format((float)old('RoomPrice', $data['room']->Price), 2, '.', '')}}" placeholder="Room Price" required @if(Auth::user()->hasRole('SuperAdmin') || Auth::user()->hasRole('user')) readonly @endif>
                        ...
                        <div class="col-md-4">
							<select class="form-control" name="RoomType"  required >
                                @foreach($data['room_types'] as $room_type)
                                    <option value="{{$room_type->name}}" @if($room_type->name == $data['room']->RoomType) selected @endif>{{$room_type->name}}</option>
                                @endforeach
					...
					<div class="form-group">
                        ...
                        <div class="col-md-4">
                            <input type="number" name="qtyofbed" class="form-control" value="{{ old('qtyofbed', $data['room']->QtyOfBed) }}" placeholder="Quantity Of Bed" required> 
						</div>
					...
					<div class="form-group">
                        ...
                        <div class="col-md-4">
							<input type="number" name="roomoccupancy" class="form-control" value="{{ old('roomoccupancy', $data['room']->MaxOccupancy )  }}" placeholder="Room Occupancy" required> 
						</div>	
							
							
						<label class="col-md-2">Room Photo</label>
						<div class="col-md-2">
							<img style="height:100%;width:100%;" src="{{asset('website').'/'.$data['room']->RoomImage}}" > 
						</div>
                        <div class="col-md-2">
					...
					<div class="form-group">
						...
						<label class="col-md-3">Quantity Of Rooms Availabl in this Room Type</label>
							...
							<div class="col-md-3">
								<input type="number" name="qtyofroom" class="form-control" value="{{ old('qtyofroom', $data['room']->AvailableQty) }}" placeholder="Quantity Of Room" required> 
							</div>
							
						<label class="col-md-3">Rooms Tax and Charges</label>
							
							<div class="col-md-3">
								<input type="number" name="roomoftax" class="form-control" value="{{ old('roomoftax', $data['room']->TaxAndCharges) }}" placeholder="Rooms Tax and Charges" required> 
							</div>
					...
					<div class="form-group">
						...
						<label class="col-md-12">Room Description</label>
							...
							<textarea maxlength="500" name="roomdescription" class="form-control" id="summernote" row="4">{!!  old('roomdescription', $data['room']->RoomDescription ) !!}</textarea>
					...
					<div class="form-group">
						...
						@foreach($data['RoomFeatureList'] as $roomfeatures)
							<div class="col-sm-4">
									<input class="form-check-input" name="roomfeature[]" @foreach($data['room']->roomFeatureList as $myroomfeature) @if($roomfeatures->FeatureID == $myroomfeature->FeatureID) checked @endif @endforeach type="checkbox" value="{{$roomfeatures->id}}" id="defaultCheck1">
